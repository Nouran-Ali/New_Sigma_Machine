...
export const getStaticPaths = async () => {
  const paths = machinemanuals.map((a) => ({
    params: { slug: a.slug },
  }));
...
export const getStaticProps = async ({ params }) => {
  const article = machinemanuals.find((a) => a.slug === params?.slug);

  if (!article) return { notFound: true };

  // ترتيب المقالات وحساب المقال السابق واللاحق والمقالات ذات الصلة
  const sortedBlogs = [...machinemanuals].sort((a, b) => a.id - b.id);
      ...
      <article className="prose mx-auto pb-10 px-24 max-lg:px-5" dir={language === "en" ? "ltr" : "rtl"}>
        <div className="bg-[#101010] rounded-2xl overflow-hidden shadow-lg border border-[#242424]">
          ...
          <div className="p-6 md:p-8">
            ...
            <p className="text-[#a2a2a3] text-lg mb-6 border-b border-[#242424] pb-5">
              {language === "en" ? article.excerpt : article.excerpt_ar}
            </p>

            <div className="text-center mb-10 bg-black py-10">
               {language === "en" ? "Download The File" : "تنزيل الملف"}
            </div>
